...
function [pool] = make_pool(num_partitions, num_levels, protate, regular, randrs)
	...
	for i = 1:num_partitions
		pool{i} = make_partition(num_levels, protate, regular, randrs);
	end
end
...
function [partition] = make_partition(num_levels, protate, regular, randrs)
	% for i levels, we need a total of 2^i -1 cuts
	for level = 0:num_levels-1
		partition(level+1) = make_cut_fracs(level, protate, regular, randrs);
	end
end
...
function [cut_fracs] = make_cut_fracs(level, protate, regular, randrs)
	...
	if regular
  	...
  	for i = 1:2^(level-1)
  		...
  		if (rand <= protate)
  			newxcut = [0 randrs.x.get() randrs.x.get()];
  			newycut = [randrs.y.get() 0 randrs.y.get()];
  			newzcut = [randrs.z.get() randrs.z.get() 0];
