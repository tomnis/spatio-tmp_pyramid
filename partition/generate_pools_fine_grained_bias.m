...
function pools = generate_pools_fine_grained_bias(num_pools, pool_size, num_levels, protate, bias_type, obj_types, regular, dataset)
	
	% TODO should i compute the distribution on the train, or all data?
	if bias_type > 1
		distr = dataset.compute_obj_distrs_fine_bias(10, obj_types);
	else
		distr.bx = [];
		distr.by = [];
		distr.bz = [];
