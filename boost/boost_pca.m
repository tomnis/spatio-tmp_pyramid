...
% and mean vectors computed for each partition scheme
function [f mean_vecs] = boost_pca(dataset, pool, target_accuracy, dim, kernel_type)
  spatial_cuts = dim.spatial_cuts;
	num_eigenvecs = dim.num_eigenvecs;
  ...
  for prt_num = 1:length(pool)
  	...
  	% must be in the loop because the histogram will be different depending on the partition pattern
		...
		mean_vec = mean(hists, 2);
		mean_vecs(:, prt_num) = mean_vec;
		
		% subtract mean vector from all observations
		hists = hists - repmat(mean_vec, 1, size(hists, 2));
    ...
    y_train1 = y_train(:, f3);
		...
		s = (x_train1 * x_train1') ./ size(x_train1,2);	
		disp 'computing eigenmatrix'
		[U, S, V] = svd(s);
		eigenspace = U(:,1:num_eigenvecs);
		x_train1 = eigenspace' * x_train1;
		%%%+++
