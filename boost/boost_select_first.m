...
function [f] = boost_select_first(dataset, pool, target_accuracy, dim, kernel_type, omit_base)
  ...
  while accuracy < target_accuracy && j < 30
  	...
  	for pattern_ind = 1:length(pool)
		...
		% force the first pattern to get selected
		if j == 1
			cur_err = dot(weights, indicators{1});
			min_pat_ind = 1;
