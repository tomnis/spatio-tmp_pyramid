...
function [accuracies] = boost_main_ramanan(pools, traindata, testdata, kernel_type, dim)
	...
	for itr=1:num_itrs
		...
		f = boost_ramanan(traindata, pool, target_accuracy, dim, kernel_type);
		...
		for pat_ind = 1:length(f.min_pat_inds)
			...
			partitioned_feats{pool_num} = testdata.compute_ramanan_histograms('pyramid'); 
		end
