...
function [f] = boost_maxerr(dataset, pool, target_accuracy, dim, kernel_type, omit_base)
	fprintf(1, 'selecting partitions with max training error\n');
  ...
  while accuracy < target_accuracy && j < 30
  	...
  	% compute the pattern which gives max err
  	max_err = -1;
  	max_pat_ind = 1;
  	for pattern_ind = 1:length(pool)
  		...
  		% if we find a new maximum, store the error, pattern, and indicator
  		if cur_err > max_err
  			max_err = cur_err;
  			max_pat_ind = pattern_ind;
  			max_indicator = indicators{pattern_ind};
  		end
  	end
  
  	% compute the weight for the pattern with max error
  	f.alpha(j) = log((1 - max_err) / max_err) + log(c - 1);
  	% remember which svm gave the max error in the jth iteration
  	f.min_class_classifiers(j) = classifiers(max_pat_ind);
  	f.min_pat_inds(j) = max_pat_ind;
  
  	assert(length(weights) == length(max_indicator));
  
  	% recompute the weights
  	% note that indicator needs to be in the correct state
  	for i=1:length(weights)
  		weights(i) = weights(i) * exp(f.alpha(j) * max_indicator(i));
  	end
