...
function [f] = boost_select_first_mult_pats(dataset, pool, target_accuracy, dim, kernel_type, omit_base)
  ...
  while accuracy < target_accuracy && j < 30
  	...
  	for pattern_ind = 1:length(pool)
		...
		%%%% 
		% hard code selecting the ramanan-style partition on the first boost round
		% let the weight be determined normally
	  if j == 1
			min_pat_ind = 1;
			min_err = pool_errs(1);
			min_indicator = indicators{1};
		else
